...
export const setUserScores = onRequest(async (request, response) => {
  // Initialize the wallet using Coinbase SDK
  const wallet = await Wallet.create(); // Create a new wallet
  console.log(`Wallet successfully created: `, wallet.toString());
  ...
  try {
    // Use the Coinbase SDK to interact with the contract
    const contract = new ethers.Contract(
      CONTRACT_ADDRESS,
      TicketProtocolImplementation.abi
    );

    // Fetch the deployed code (optional)
    const code = await contract.getDeployedCode();
    logger.info("Contract code", { code });

    // Set fan scores using the contract
    const tx = await contract.setFanScores(
      request.body.users, // Use users from the request body
      request.body.scores // Use scores from the request body
    );
