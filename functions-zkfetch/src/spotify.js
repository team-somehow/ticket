...
async function getSpotifyData(accessToken, artistName) {
    ...
    const searchParams = new URLSearchParams({ q: artistName, type: 'artist', limit: 1 });
    const searchUrl = `https://api.spotify.com/v1/search?${searchParams.toString()}`;
    const searchResults = await zkFetchJson(searchUrl, { method: 'GET', headers });
    ...
    const topArtistsItems = (await zkFetchJson(topArtistsUrl, { method: 'GET', headers })).items || [];
    ...
    const topTracksItems = (await zkFetchJson(topTracksUrl, { method: 'GET', headers })).items || [];
    ...
    const followsArtist = (await zkFetchJson(followsUrl, { method: 'GET', headers }))[0];

    // Get user's playlists and their tracks
    let playlists = [];
    let nextUrl = "https://api.spotify.com/v1/me/playlists?limit=20";
    while (nextUrl) {
        const playlistsResponse = await zkFetchJson(nextUrl, { method: 'GET', headers });
    ...
    for (const playlist of playlists) {
        try {
            ...
            while (tracksUrl) {
                const tracksResponse = await zkFetchJson(tracksUrl, { method: 'GET', headers });
    ...
    const listensToSimilarGenres = artistGenres.some(genre => userGenres.has(genre));
